...
bool PR2NeuroadptControllerClass::init( pr2_mechanism_model::RobotState *robot, ros::NodeHandle &n )
{
  ...
  if (!n.getParam( nn_kappa            , kappa            )) { ROS_ERROR("Value not loaded from parameter: %s !)", nn_kappa.c_str())			; return false; }
  if (!n.getParam( nn_Kv               , Kv               )) { ROS_ERROR("Value not loaded from parameter: %s !)", nn_Kv.c_str())				; return false; }
