...
bool construct_float_model (Model* rbdl_model, urdf::Model *urdf_model, bool verbose, string baseLink) {
	...
	for (j = 0; j < joint_names.size(); j++) {
		...
		unsigned int rbdl_parent_id = 6;

                if (urdf_parent->name == baseLink && rbdl_model->mBodies.size() == 1)
                {
                        // assemble the body
                        ...
                        if (urdf_parent->inertial) {
                                link_inertial_mass = urdf_parent->inertial->mass;

                                link_inertial_position.set (
                                                urdf_parent->inertial->origin.position.x,
                                                urdf_parent->inertial->origin.position.y,
                                                urdf_parent->inertial->origin.position.z
                                                );
                                urdf_parent->inertial->origin.rotation.getRPY (link_inertial_rpy[0], link_inertial_rpy[1], link_inertial_rpy[2]);

                                link_inertial_inertia(0,0) = urdf_parent->inertial->ixx;
                                link_inertial_inertia(0,1) = urdf_parent->inertial->ixy;
                                link_inertial_inertia(0,2) = urdf_parent->inertial->ixz;

                                link_inertial_inertia(1,0) = urdf_parent->inertial->ixy;
                                link_inertial_inertia(1,1) = urdf_parent->inertial->iyy;
                                link_inertial_inertia(1,2) = urdf_parent->inertial->iyz;

                                link_inertial_inertia(2,0) = urdf_parent->inertial->ixz;
                                link_inertial_inertia(2,1) = urdf_parent->inertial->iyz;
                                link_inertial_inertia(2,2) = urdf_parent->inertial->izz;

                                if (link_inertial_rpy != Vector3d (0., 0., 0.)) {
                                        cerr << "Error while processing body '" << urdf_parent->name << "': rotation of body frames not yet supported. Please rotate the joint frame instead." << endl;
                                        return false;
                                }
                        }
                        ...
                        rbdl_model->SetFloatingBaseBody(rbdl_body);
                }
...
//		cout << "joint: " << urdf_joint->name << "\tparent = " << urdf_parent->name << " child = " << urdf_child->name << " parent_id = " << rbdl_parent_id << endl;
		...
		if (urdf_joint->type == urdf::Joint::REVOLUTE || urdf_joint->type == urdf::Joint::CONTINUOUS) {
			...
			cout << "Added: " << urdf_joint->name << " " << urdf_parent->name << " " << urdf_joint->axis.x << " " << urdf_joint->axis.y << " " << urdf_joint->axis.z << endl;
...
bool read_urdf_float_model (const char* filename, Model* model, bool verbose, string baseLink) {
	assert (model);
	...
	if (!construct_float_model (model, &urdf_model, verbose, baseLink)) {
...
bool construct_model (Model* rbdl_model, urdf::Model *urdf_model, bool verbose, string baseLink) {
        boost::shared_ptr<urdf::Link> urdf_root_link;
        URDFLinkMap link_map;
        link_map = urdf_model->links_;
        URDFJointMap joint_map;
...
bool read_urdf_model (const char* filename, Model* model, bool verbose, string baseLink) {
        assert (model);
        urdf::Model urdf_model;
        cerr << "Warning: this code (RigidBodyDynamics::Addons::" << __func__ << "()) is not properly tested as" << endl;
        cerr << "         I do not have a proper urdf model that I can use to validate the model loading." << endl;
        ...
        if (!construct_model (model, &urdf_model, verbose, baseLink)) {
                cerr << "Error constructing model from urdf file." << endl;
                return false;
        }
        model->gravity.set (0., 0., -9.81);
